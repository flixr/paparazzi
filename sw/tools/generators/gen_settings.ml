...
let _ =
  ...
  let md5sum = try Sys.getenv "MD5SUM" with _ -> "" in
  ...
  try
    ...
    let settings_attribs = if md5sum <> "" then [("md5sum", md5sum)] else [] in
    let xml = Xml.Element ("settings", settings_attribs, [rc_settings; dl_settings]) in
